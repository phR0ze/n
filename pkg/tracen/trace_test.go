...
func TestCallerTrace(t *testing.T) {
	...
	assert.Equal(t, "tracen.AppFunc3", trace[0].Func)
	assert.Equal(t, "tracen.AppFunc2", trace[1].Func)
	assert.Equal(t, "tracen.AppFunc1", trace[2].Func)
	assert.Equal(t, "tracen.TestCallerTrace", trace[3].Func)
}
...
func TestCallerTraceOne(t *testing.T) {
	...
	assert.Equal(t, "tracen.LogFunc4", trace[0].Func)

	// Test with single skip file
	skipfiles = append(skipfiles, "tracelog2_test")
	AppFunc1()
	assert.Len(t, trace, 1)
	assert.Equal(t, "tracen.LogFunc2", trace[0].Func)
